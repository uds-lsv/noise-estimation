...
class ExperimentalSettings:
    ...
    def __setitem__(self, key, value):
        warnings.warn("You are CHANGING the settings")
        if key in self.settings:
            self.logger.info(f"default value for key {key}:  {self.settings[key]}")
        else:
            self.logger.info(f"default value for key {key}:  None")
        self.logger.info(f"updated value for key {key}:  {value}")
    ...
    @staticmethod
    def load_json(name, logger, dir_path="./trainer_config/"):
        with open(os.path.join(dir_path, name + ".json"), 'r') as f:
            ...
            new_settings_object = ExperimentalSettings(name, logger)
