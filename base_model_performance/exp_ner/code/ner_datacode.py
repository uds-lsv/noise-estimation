...
class WordEmbedding:
    ...
    def get_emb(self, instance):
        word_emb = self.word_to_embedding(instance.word)
        left_context_emb = [self.word_to_embedding(word) for word in instance.left_context]
        right_context_emb = [self.word_to_embedding(word) for word in instance.right_context]
        return left_context_emb, word_emb, right_context_emb
        
    def get_emb_for_instances(self, instances):
        for instance in instances:
            l, w, r = self.get_emb(instance)
            emb = []
            emb.extend(l)
            emb.append(w)
            emb.extend(r)
            instance.embedding = np.asarray(emb)
...
class LabelRepresentation:
    ...
    def use_connl_normalized_labels(self):
        self.use_specific_label_map({"O": 0, "PER": 1, "ORG": 2, "LOC": 3})
    ...
    def get_lbemb_for_instances(self, instances):
        for instance in instances:
    ...
    def predictions_to_labels(self, predictions):
        ...
        return [self.label_idx_to_label_name(label_idx) for label_idx in predictions]
...
class Evaluation:
    ...
    def create_connl_evaluation_format(self, instances, words, prediction_labels):
